...
class Attention(nn.Module):
    def __init__(self, dim, num_patches, num_tokens=1, num_heads=8, channel=None, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        # token_mask = torch.rand(num_patches + num_tokens)
        self.token_index = nn.Parameter((torch.ones(num_patches + num_tokens) == 1), requires_grad=False)
        # self.token_index = nn.Parameter((token_mask), requires_grad=False)
    ...
    # def compute_rank_attn(self, grad):
    #     values = torch.sum(grad * self.attn, dim=0, keepdim=True) \
    #                   .sum(dim=1, keepdim=True).sum(dim=3, keepdim=True)[0, 0, 1:, 0].data
    #     # print(values.shape)
    #     # print('attn')
    #     values = values / (self.attn.size(0) * self.attn.size(1) * self.attn.size(3))
    #     if self.seq_ranks is None:
    #         self.seq_ranks = torch.zeros(grad.size(2) - 1).cuda()
    #
    #     self.seq_ranks += torch.abs(values)
...
class Block(nn.Module):
    ...
    def forward(self, x, compute_taylor_attn=False, compute_head_entro=False):
        ...
        token_keep = token_keep + self.drop_path(self.attn(self.norm1(token_keep), compute_taylor_attn, compute_head_entro))
        token_keep = token_keep + self.drop_path(self.mlp(self.norm2(token_keep)))
        x = torch.cat([token_keep, token_res], dim=1)

        # clip & concat
        # x = x[:, self.attn.token_index, :]
        # x = x[:, :self.attn.channel, :]
        # # print(token_keep.shape)
        # x = x + self.drop_path(self.attn(self.norm1(x), compute_taylor_attn, compute_head_entro))
        # x = x + self.drop_path(self.mlp(self.norm2(x)))
        # x[:, :self.attn.channel, :] = x
        # print(x.shape)
        # assert 0

        # shortcut
        # x_ = x[:, self.attn.token_index, :]
        # x_ = x_ + self.drop_path(self.attn(self.norm1(x_), compute_taylor_attn, compute_head_entro))
        # x_ = x_ + self.drop_path(self.mlp(self.norm2(x_)))
        # x = x[:, self.attn.token_index, :] + x_
        # x[:, self.attn.token_index, :] = x[:, self.attn.token_index, :] + self.drop_path(
        #                                 self.attn(self.norm1(x[:, self.attn.token_index, :]), compute_taylor_attn, compute_head_entro))
        # x[:, self.attn.token_index, :] = x[:, self.attn.token_index, :] + self.drop_path(
        #                                 self.mlp(self.norm2(x[:, self.attn.token_index, :])))

        # argsort
        # _, shuffle = torch.sort(self.attn.token_index)
        # x = x[:, shuffle, :]
        # x = x[:, :self.attn.channel, :]
        # x = x + self.drop_path(self.attn(self.norm1(x), compute_taylor_attn, compute_head_entro))
        # x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, embed_dims=None, depth=12,
        ...
        for i in range(depth):
            self.blocks.append(
                Block(
                    dim=embed_dims[i], num_heads=num_heads[i], num_patches=self.num_patches, num_tokens=self.num_tokens,
