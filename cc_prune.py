...
from vit_model import VisionTransformer, Attention
...
def taylor_prune(model, nums, mode='head_entropy'):
    layer_ranks = []
    attn_maps = []
    iter_cnts = []
    if mode == 'head':
    ...
    # print(layer_ranks)`q
    layer_ranks = np.asarray([np.asarray(layer_rank.cpu()) * (1 - 0.005 * layer)
